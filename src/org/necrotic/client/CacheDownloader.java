...
/*
	...
	public static final String MIRROR_URL_TO_LOADING_IMAGES = "http://necrotic.org/downloads/images/"; // If first link
																										// is broken, it
																										// will attempt
																										// to download
																										// from here
*/
	 ...
	 private static final String CACHE_FILE_NAME = "NecroticCache5.zip";
	 //private static final String CACHE_URL = "https://www.dropbox.com/s/nzgib5u6bjgtvwf/NecroticCache.zip?dl=1";
	 private static final String CACHE_URL = "https://www.dropbox.com/s/ndzr51rb72yrfha/NecroticCache5.zip?dl=1";
			...
			if (cacheDownloadRequired(newest, current) || forceUpdateCache()) {
				if (Configuration.STOP_CACHE_UPDATES) {
					...
					if (forceUpdateCache() == true) {
						...
						System.out.println(
								"Naturally found cache update. No manual overrides detected, proceeding as normal. Current: "
	...
	public static void downloadCache() throws IOException {
		...
		if (responseCode == HttpURLConnection.HTTP_OK) {
			...
			while ((bytesRead = inputStream.read(buffer)) != -1) {
				...
				// int downloadSpeed = (int) ((downloaded / 1024) / (1 +
				// ((System.currentTimeMillis() - startTime) / 1000)));
				// Client.getClient().drawSmoothLoading(percentage, (new
				// StringBuilder()).append("Downloading "+percentage+"% ").append("@
				// "+downloadSpeed+"Kb/s").toString());
				// System.out.println((new StringBuilder()).append("Downloading "+percentage+"%
				// ").append("@ "+downloadSpeed+"Kb/s").toString());
				// drawLoadingText(percentage, (new StringBuilder()).append("Downloading
				// "+downloadingText+""+s+": "+percentage+"% ").append("@
				// "+downloadSpeed+"Kb/s").toString());
			}
