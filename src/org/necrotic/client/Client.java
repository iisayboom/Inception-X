...
public class Client extends GameRenderer {
    ...
    private String[] menuActionTooltip;
    ...
    public Client() {
        ...
        menuActionTooltip = new String[500];
        Arrays.fill(menuActionTooltip, "");
    ...
    private void drawHover(int draw_x, int draw_y) {
        if (menuActionRow - 1 < 0) {
            return;
        }
        hover_tooltip = menuActionName[menuActionRow - 1];
        if (hover_tooltip.startsWith("@whi@View")) {
            return;
        }
        if (hover_tooltip.toLowerCase().contains("walk here")) {
            return;
        }
        hover_tooltip += menuActionTooltip[menuActionRow - 1];
    ...
    private void buildMenu(String desc, int draw_x, int draw_y) {
        ...
        hover_tooltip = RSFontSystem.handleOldSyntax(hover_tooltip);
        for (String line = hover_tooltip; line.length() > 0; ) {
            ...
            String reducedWith = new_line.replaceAll("<.*?>", "");
            int new_width = font.getTextWidth(reducedWith);
            if (new_width > menu_width) {
                menu_width = new_width;
            }
            menu_hegiht += font.getBaseCharacterHeight() + 2;
        }
        menu_width += 8;
        menu_hegiht += 9;
        int client_width = clientWidth;
        ...
        DrawingArea.drawRectangle(draw_y + 1, menu_hegiht - 2, 170, 0, menu_width + 3, draw_x + 1);
        DrawingArea.drawRectangle(draw_y, menu_hegiht, 170, 0, menu_width + 1, draw_x + 2);
        DrawingArea.drawRectangle(draw_y + 1, menu_hegiht - 2, 256, 0xffffff, menu_width - 1, draw_x + 3);
        DrawingArea.drawRectangle(draw_y + 2, menu_hegiht - 4, 256, 0xffffff, menu_width + 1, draw_x + 2);
        DrawingArea.fillRectangle(0x222222, draw_y + 2, menu_width - 1, menu_hegiht - 4, 220, draw_x + 3);
        for (int new_y = draw_y + font.getBaseCharacterHeight() + 4; hover_tooltip.length() > 0; new_y += font.getBaseCharacterHeight() + 2) {
            ...
            font.drawBasicString(desc, draw_x + 5, new_y, 0x999999, -1);
        }
    }
            ...
            if (face == 2 && worldController.fetchObjectIDTagForPosition(plane, x, y, modelData) >= 0) {
                ...
                if (itemSelected == 1) {
                    ...
                    menuActionName[menuActionRow] = myRights >= 60 && myRights <= 69 ? "Examine @cya@"
                            ...
                            + "@gre@)" : "Examine @cya@"
                            + class46.name;
                    menuActionTooltip[menuActionRow] = "\\n"+class46.name + " @whi@(@gre@"+class46.type
                    + "@whi@, @gre@" + (x + baseX) + " @whi@, @gre@"+ (y + baseY) + "@whi@)";
                    menuActionID[menuActionRow] = 1226;
										...
										*/
                                        ...
                                        if (openInterfaceID != 24700) {
                                            if (!children.hideExamine) {
                                                ...
                                                menuActionTooltip[menuActionRow] = "\\n" +
                                                        definition.name + " @whi@(@gre@"+definition.id+"@whi@)"
                                                + "\\nModels@whi@(@gre@"+definition.modelID+"@whi@,@gre@"
                                                + definition.femaleWearId + "@whi@,@gre@"+definition.maleWearId+"@whi@)";
